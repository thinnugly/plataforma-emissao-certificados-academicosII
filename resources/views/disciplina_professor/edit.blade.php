                ...
                <div class="card card-default">
                    <div class="card-header">
                        ...
                        <div class="float-right">
                            <a class="btn btn-warning getBack" href="{{ route('_professor') }}"><i class="fa fa-fw fa-arrow-left"></i> Retroceder</a>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('_updateAloc', ['id' => $disc_prof->id]) }}"  role="form" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" id="getDisciplinaId">
                            <input type="hidden" id="getDisciplinaIdII">
                            <input type="hidden" class="serdelete_val_disname" value="{{ $disc_prof->disciplinaId}}">
                            <input type="hidden" class="serdelete_val_disid" value="{{ $disc_prof->id}}">
                            <div class="row" style="margin-top: 10px;">
                                <div class="col-md-3">
                                    ...
                                    <select id="classeId" class="form-control getClasse @error('classeId') is-invalid @enderror" name="classeId" value="{{ old('classeId') }}">
                                        <option selected>{{ $disc_prof -> classeId}}</option>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select id="seccaoId" class="form-control getSeccao @error('seccaoId') is-invalid @enderror" name="seccaoId" value="{{ old('seccaoId') }}">
                                        <option selected>{{ $disc_prof -> seccaoId}}</option>
                                    </select>
                                    @error('seccaoId')
                                    <span class="invalid-feedback" role="alert">
                                          <strong>{{ $message }}</strong>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id="turmaId" class="form-control getTurma @error('turmaId') is-invalid @enderror" name="turmaId" value="{{ old('turmaId') }}">
                                        <option selected>{{ $disc_prof -> turmaId}}</option>
                                    </select>
                                    @error('turmaId')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                            ...
                            <div class="row" style="margin-top: 10px;">
                                <div class="col-md-5">
                                    ...
                                    <select id="disciplinaId" class="form-control getDisciplina @error('disciplinaId') is-invalid @enderror" name="disciplinaId" value="{{ old('disciplinaId') }}">
                                        <option selected>{{ $disc_prof -> disciplinaId}}</option>
                                    </select>
                                    @error('disciplinaId')
                                    <span class="invalid-feedback" role="alert">
                                           <strong>{{ $message }}</strong>
                                ...
                                <div class="col-md-7">
                                    ...
                                    <select id="professorId" class="form-control getProfessor @error('professorId') is-invalid @enderror" name="professorId" value="{{ old('professorId') }}">
                                        <option selected>{{ $disc_prof -> professorId}}</option>
...
<script type="text/javascript">
    $(document).ready(function ()
    {
        ...
        var getItemName = $('.serdelete_val_disname').val();
        //var getItemid = $('.serdelete_val_disid').val();
        ...
        $.ajax(
            {
                ...
                success: function (data){
                    for(var i=0; i<data.length; i++){
                        $('#getDisciplinaIdII').val(data[i].id);
                        //var getItemId = $('#getDisciplinaIdII').val(); 
                        //console.log(getItemId)  
                    }
                },
                error: function (){
        ...
        $(document).on('click', '.actualizarStatusTurmaDis', function()
        {
            ...
            var getItemid = $('.serdelete_val_disid').val(),
                getProfessorId = $('#professorId').val();
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            ...
            $.ajax({
                type: 'get',
                url: '{!! URL::to('/administrator/disciplinas/update') !!}',
                data: data,
                success: function(response)
                { 
                    if(response.estado == 200)
